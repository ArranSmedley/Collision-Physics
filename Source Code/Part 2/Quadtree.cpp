...
void Quadtree::AddObject(RigidBody* object) {
	if (level == maxLevel) {
...
vector<RigidBody*> Quadtree::GetObjectsAt(float _x, float _y) {
	if (level == maxLevel)
		return objects;

	vector<RigidBody*> returnObjects, childReturnObjects;
	if (!objects.empty()) {
...
bool Quadtree::contains(Quadtree* child, RigidBody* object) {
	return	 !(object->getPos().x < child->x ||
		object->getPos().y < child->y ||
		object->getPos().x  > child->x + child->width ||
		object->getPos().y > child->y + child->height ||
		object->getPos().x + object->getRadius() < child->x ||
		object->getPos().y + object->getRadius() < child->y ||
		object->getPos().x + object->getRadius() > child->x + child->width ||
		object->getPos().y+ object->getRadius() > child->y + child->height);
}
